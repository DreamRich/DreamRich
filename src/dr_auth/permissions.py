...
class FinancialAdvisersPermission(permissions.BasePermission):
    def has_permission(self, request, view):
        if request.user.is_anonymous():
            return True
        elif checkers.has_permission(request.user, 'see_financial_adviser_data'):
            return True
        return False
...
class EmployeesPermission(permissions.BasePermission):
    def has_permission(self, request, view):
        if request.user.is_anonymous():
            return True
        elif checkers.has_permission(request.user, 'see_employee_data') \
...
class ClientsPermission(permissions.BasePermission):
    def has_permission(self, request, view):
        if request.user.is_anonymous():
            return True
        elif checkers.has_permission(request.user, 'change_own_client_data'):
            return True
        return False
